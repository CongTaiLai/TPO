...
response = requests.get('https://top.zhan.com/toefl/read/alltpo.html', headers=headers)

soup = BeautifulSoup(response.text, 'html.parser')

raw = soup.find('ul', {'class': 'cssTopTitleList clearfix'})

links = []

for a in raw.find_all('a'):
    links.append(a['href'])

for e in exam_sections:
    l_ = []
    with open(f'{e}_contents.txt', 'w') as file:
        for l in links:
            # contents only differ in exam sections
            l_.append(l.replace('read', e))
        file.write(str(l_))
